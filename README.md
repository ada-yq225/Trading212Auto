...
`auto_trader.py` 使用 `rational_momentum_ml_v4_fast` 多因子长仓模型：

- 每 15 秒采样，分别计算约 5、15 和 45 分钟的对数收益。
...
- 最多每 5 分钟再平衡一次，同一股票交易后冷却 5 分钟；偏差小于资产的 1.5% 或 £20 时不交易，单笔最多约 £300。
...
当前主策略股票记录在 `universe.json`。加速配置启动后约 30 分钟形成在线实验模型的最小实时历史，约 45 分钟形成主策略的完整长周期窗口；预热期间不会根据不完整信号追涨，只会处理硬止损和超过 30% 的集中仓位。策略升级时会清空旧模型的价格样本，避免把不同采样频率的数据混在一起。

市场休市时程序每 60 秒检查一次是否重新开盘，不提交订单。开盘时每 15 秒运行一次；网络或 API 失败时使用最长 5 分钟的指数退避，并在恢复后继续运行，避免断网期间高频空转。

### 自动市场侦察

`universe.json` 维护 50 只股票、11 个板块的候选池：8 只原有股票继续运行主策略，另外 42 只作为侦察标的。开盘期间最多每 5 分钟新增一个约 £1.35–£2.50 的 Demo 探针仓，并始终保留策略要求的现金缓冲。候选股至少积累 241 个 15 秒样本（约 1 小时）后，使用与主策略相同的波动率标准化动量评分；评分达到 0.75 且长周期收益为正的最强股票才会晋升。
...
实验系统在同一份因果数据上并行评估四个结构不同的候选：

- `legacy_ensemble`：标准化 Ridge 与限制深度的直方图梯度提升树集成。
- `robust_huber`：RobustScaler 与 Huber 损失随机梯度回归，降低异常值影响。
- `regime_histgb`：使用股票和市场状态全部 16 个特征的非线性梯度提升模型。
- `residual_momentum`：股票相对市场的多周期残差动量规则，不依赖拟合估计器。

每个候选使用相同的 12 个股票因果特征和 4 个同步市场上下文特征。特征只读取预测时点及之前的价格；训练标签延迟 15 个样本（当前为 225 秒），未成熟的未来收益不会进入训练集。预测批次也固定为每 225 秒一次，因此同一候选的前向结果不重叠。四个候选共享一次数据构建，每个 15 秒主循环最多使用 8 秒实验计算预算；单个候选报错或超时不会阻塞其他候选或交易风控。

如果程序启动时发现距离上次采样超过 5 分钟，会清空不再连续的 15 秒价格样本、过期预测和冠军状态，重新完成连续预热；已有前向结果、冻结结论、持仓峰值、账户与订单计数都会保留。这样不会把停机数天的价格变化误当作一个 15 秒收益，也不会让停机前的旧预测在重启后直接影响交易。

候选默认处于 `WARMUP`，训练样本足够后进入 `SHADOW`，只记录预测而不影响订单。最近至少 20 个独立预测批次、40 个入选结果的方向命中率达到 53%，并且扣除 0.1% 假设往返成本后的平均收益为正，才可能进入 `APPROVED`。挑战者积累至少 10 批、20 个结果后，只有当命中率和净收益的一侧 95% 上界都低于准入门槛时才会提前冻结；早停不能批准模型，原有模型也不会被早停。

系统在所有已批准候选中按保守净收益（平均值减标准误）选择唯一冠军。只有这个冠军最多影响综合评分的 20%；原有仓位、板块、回撤和止损限制仍然有效。冠军指标恶化时会立即撤销并退回影子模式。锦标赛只消费主循环已经取得的内存价格，不直接访问 Trading 212 客户端，也没有独立下单路径。
